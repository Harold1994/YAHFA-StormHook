...
public class HookManager {
    ...
    public void applyHooks(Class<?> holdClass) {
        ...
        for (Method hookMethod : holdClass.getDeclaredMethods()) {
            Log.d("Hook","2: find hook method " + hookMethod.toString());
            Hook hook = hookMethod.getAnnotation(Hook.class);
            if (hook != null) {
                String statement = hook.value();
                String[] splitValues = statement.split("::");//android.app.Activity::startActivity@android.content.Intent"
                        ...
                        try {
                            ...
                            for (Method method : clazz.getDeclaredMethods()) {
                                if (method.getName().equals(methodName)) {
                                    Log.d("Hook","3: " + method.getName() + " equals " + methodName);
                                    ...
                                    if (paramList.length == types.length) {
                                        ...
                                        if (isMatch) {
                                            Log.d("Hook","4: " +"is match,begin to hookMethod");
    ...
    public void hookMethod(Method origin, Method hook) {
        ...
        Method backupMethod;
        if (Runtime.isArt()) {
            Log.d("Hook","3: begin hookMethodArt");
        ...
        Map<String,List<Method>> methodNameToBackupMethodsMap = classToBackupMethodsMapping.get(className);//备份原始方法
    ...
    private static Method hookMethodArt(Method origin, Method hook) {
        Log.d("hookart","************************");
        Log.d("hookart","begin to hook art method");
