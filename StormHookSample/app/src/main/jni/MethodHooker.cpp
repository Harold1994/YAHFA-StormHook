...
int makeDexElements(JNIEnv* env, jobject classLoader, jobject dexFileobj)
{
	...
	//???elements???? get dexElement array value
	...
	//??????????dalvik/system/DexPathList$Element?? dexFileobj????dexFileobj
...
jclass myFindClass(JNIEnv* jenv,const char* targetClassName,jobject dexObj)
{
	...
	ALOG("storm","begin to find target class");
	jclass clazzTarget = jenv->FindClass(targetClassName);
	ALOG("storm","after find target class");
	if (ClearException(jenv)) {
...
int Hook(){
    ...
    const char* pkgName="com.example.stormhooksample";
